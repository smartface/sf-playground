...
export default class PgFlexLayout extends withDismissAndBackButton(PgFlexLayoutDesign) {
    ...
    showPickerForJustifyContent() {
        ...
        const items = ['CENTER', 'FLEX_START', 'FLEX_END', 'SPACE_AROUND', 'SPACE_BETWEEN', 'SPACE_EVENLY'];
        picker.items = items;
    ...
    changeWrap() {
        const picker = new Picker();
        const items = ['NOWRAP', 'WRAP', 'WRAP_REVERSE'];
        picker.items = items;
        picker.on('selected', (index) => {
            console.info('selected: ', index);
            this.fl.dispatch({
    ...
    showPickerForAlignContent() {
        const picker = new Picker();
        const items = ['AUTO', 'CENTER', 'FLEX_START', 'FLEX_END', 'STRETCH', 'BASELINE', 'SPACE_BETWEEN', 'SPACE_AROUND'];
        picker.items = items;
        picker.on('selected', (index) => {
            console.info('selected: ', index);
            this.fl.dispatch({
